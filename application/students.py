...
def student_class_join():
	execute_db("INSERT INTO roster (people_id, class_id) VALUES (?, ?);", [session['id'], request.form['id']])
...
def student_class_page(class_id):
	...
	return render_template('/students/class_page.html', class_name=str(className[0]), topics=getClassTopics(class_id), quizzes=getClassQuizzes(class_id), grades=getStudentGrades(class_id))
...
def student_topic_page(topic_id):
	...
	return render_template('/students/topic_page.html', topic_name=str(topicName[0]),  quizzes=getTopicQuizzes(topic_id))
...
def grade_quiz(quiz_id):
	...
	execute_db("INSERT INTO quiz_grades (student_id, quiz_id, grade) VALUES (?, ?, ?);", [session['id'], quiz_id, percent])
