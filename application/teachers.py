...
def create_class():
	...
	execute_db("insert into classes (teacher_id, name) values (?, ?);", [session['id'], request.form['name']])
...
def class_page(class_id=None):
	...
	return render_template('/teachers/class_page.html', class_id=class_id, class_name=class_name, topics=getClassTopics(class_id), students=getRegisteredStudents(class_id), grades=getClassGrades(class_id))
...
@validate_teacher
def create_topic():
	execute_db("insert into topics (name, class_id) values (?, ?);", [request.form['name'], request.form['class']])
	flash("Your class was created.")
	return render_template('/teachers/topics.html', classes=getTeacherClasses())

## Delete a topic
@app.route('/teachers/topics/delete/', methods=['POST'])
@validate_teacher
def Delete_topic():
	execute_db("DELETE FROM quizzes WHERE (name=? AND creator_id=?) ;", [request.form['name'], session['id']])
	execute_db("Delete FROM topics WHERE (name=? AND class_id=?);", [request.form['name'], request.form['class']])
	flash("Your class was deleted.")
	return render_template('/teachers/topics.html', classes=getTeacherClasses())
...
def topic_page(topic_id=None):
	...
	return render_template('/teachers/topic_page.html',  topic_name=str(topicData[0]), quizzes=getTopicQuizzes(topic_id))
...
@validate_teacher
def create_quiz():
	execute_db("insert into quizzes (topic_id, creator_id, name) values(?, ?, ?);", [request.form['topic'],  session['id'], request.form['name']])
	flash("The quiz was created.")
	return redirect("/teachers/quizzes/")

## delete quiz
@app.route('/teachers/quizzes/delete/', methods=['POST'])
@validate_teacher
def delete_quiz():
	execute_db("DELETE FROM quizzes WHERE (topic_id=? AND creator_id=? AND name=?) ;", [request.form['topic'],  session['id'], request.form['name']])
...
def create_question(quiz_id=None):
	execute_db("INSERT INTO questions (correct_answer, question_text, a_answer_text, b_answer_text, c_answer_text, d_answer_text, quiz_id) VALUES (?, ?, ?, ?, ?, ?, ?);", [str(request.form['answer']), str(request.form['question']), str(request.form['a_answer']), str(request.form['b_answer']), str(request.form['c_answer']), str(request.form['d_answer']), str(quiz_id)])
